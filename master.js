...
function renderDrivers(){const el=document.querySelector('#driversTable'); if(!el)return; el.innerHTML=window.deliveryDrivers.map(d=>`<tr><td><b>${esc(d.name||'')}</b></td><td>${esc(d.phone||'')}</td><td>${esc(d.email||'')}</td><td>${d.active===false?'OFF':'ON'}</td><td><button class="danger remove-driver" data-uid="${esc(d.uid)}" data-name="${esc(d.name||d.uid)}">🗑️ Remove</button></td></tr>`).join('')||'<tr><td colspan="5">No delivery boys added yet.</td></tr>'; document.querySelectorAll('.remove-driver').forEach(b=>b.onclick=()=>removeDriver(b.dataset.uid,b.dataset.name));}
