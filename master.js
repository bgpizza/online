...
function renderOrders(snapshot){
  ...
  $("#ordersTable").innerHTML=rows.length?rows.map(o=>`<tr><td><b>${esc(o.orderId)}</b></td><td>${esc(o.name)}<br><small>${esc(o.phone)}</small><br><small>${esc(o.address)}</small></td><td>₹${Number(o.total||0).toLocaleString("en-IN")}</td><td>${Number(o.distanceKm||0).toFixed(1)} KM</td><td><select class="order-status" data-id="${esc(o.orderId)}">${STATUS_LIST.map(s=>`<option ${o.status===s?"selected":""}>${s}</option>`).join("")}</select></td><td><button class="status-wa" data-id="${esc(o.orderId)}" data-phone="${esc(o.phone)}">💬 Send Status</button></td><td>${formatDate(o.createdAt)}</td></tr>`).join(""):`<tr><td colspan="7">No orders found.</td></tr>`;
...
async function updateStatus(id,status){try{await db.collection("orders").doc(id).update({status,updatedAt:firebase.firestore.FieldValue.serverTimestamp()});await db.collection("publicStatuses").doc(id).set({status,updatedAt:firebase.firestore.FieldValue.serverTimestamp()},{merge:true});}catch(e){console.error(e);alert("Status update failed.")}}
